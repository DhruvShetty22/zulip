...
def get_comment_content(payload: WildValue, auth: JiraAuth | None = None) -> str:
    ...
    for account_id in set(re.findall(r"\[~accountid:([\w\-:]+)\]", comment)):
        display_name = get_jira_api_data(jira_api_url, "displayName", auth, accountId=account_id)
        comment = re.sub(rf"\[~accountid:{re.escape(account_id)}\]", f"**{display_name}**", comment)
    return comment
